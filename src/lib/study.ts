...
import { quizItems } from "./quiz-scoring"
import { ensureSetContent, setContentFor, synthesiseSet, useSetContentVersion } from "./set-content"
import type { MixedQuestion, StudySet } from "./study-types"

export type { Flashcard, QuizQuestion, MixedQuestion, QuizResponse, Mastery, SetStatus, StudySet } from "./study-types"
...
/**
 * A study set by id, with whatever content the device actually holds for it.
 *
 * The bundled record supplies presentation (artwork, mastery split, status copy); `lib/set-content`
 * supplies cards and questions when a download or an API fetch has installed them, and they win. That
 * is what makes a downloaded set a real thing rather than a receipt: every screen already calls this
 * function, so all of them read the downloaded content without knowing the difference.
 *
 * A set that is downloaded but not bundled (a server-only set) is synthesised from its API record, so
 * it is playable offline too. `cardsTotal` is recomputed from the cards actually present — a count
 * inherited from the bundle would be a promise the downloaded content might not keep.
 */
export function getStudySet(id: string | undefined): StudySet | undefined {
  const bundled = STUDY_SETS.find((s) => s.id === id)
  const source = setContentFor(id)
  if (!source) return bundled
  if (!bundled) return synthesiseSet(id!, { thumb: IMG.maths, hero: IMG.maths })

  const { content } = source
  return {
    ...bundled,
    cards: content.cards,
    quiz: content.quiz.length > 0 ? content.quiz : bundled.quiz,
...
/**
 * A set, resolving it over the network if the device does not already have it.
 *
 * The screens used to bounce straight `/home` when `getStudySet` returned nothing, which is wrong in
 * two different ways: a set that exists on the server but is not bundled is perfectly openable while
 * online, and a set that genuinely does not exist deserves to be told to the user rather than
 * answered with a silent teleport to the dashboard.
 *
 *   `loading` — a fetch is in flight; show a placeholder, not an empty state.
 *   `set`     — playable, from the bundle, a download, or the API.
 *   `failed`  — no local copy and the fetch did not produce one. Say so.
 */
export function useResolvedStudySet(id: string | undefined) {
  const set = useStudySet(id)
  // The outcome of the fetch, tagged with the id it was for, so a stale result cannot be read as the
  // answer for a different set. Written only from the promise callbacks: setting "loading"
  // synchronously inside the effect is what the React Compiler rejects as a cascading render, and the
  // state is derivable anyway.
  const [resolved, setResolved] = useState<{ id: string; ok: boolean } | null>(null)
  ...
  useEffect(() => {
    ...
    ensureSetContent(id)
      .then((ok) => {
        if (active) setResolved({ id, ok })
      })
      .catch(() => {
        // The API client has already reported the reason; the screen only needs to know it failed, and
        // an offline device failing here is expected rather than exceptional.
        if (active) setResolved({ id, ok: false })
      })
    return () => {
      active = false
    }
  ...
  const settled = !!id && resolved?.id === id
  return { set, loading: !set && !!id && !settled, failed: !set && settled && resolved?.ok === false }
}

/**
 * The same lookup, subscribed to content changes. Screens that render a set's cards or questions use
 * this so a download finishing (or being deleted, or being hydrated off disk after the screen
 * mounted) re-renders them instead of leaving stale content on screen.
 */
...
/**
 * Quiz grading lives in `./quiz-scoring` — a leaf module with no asset `require`s, so it can be unit
 * tested without loading this catalogue. Re-exported here because every screen imports it from
 * `@/lib/study`.
 */
export {
  quizItems,
